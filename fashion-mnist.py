...
#reshape images to (num_samples, 28, 28, 1), since images are one channel.
...
#augmented_data_gen.fit(train_images)

print("Dataset Stats")
print("Training images shape: {}".format(train_images.shape))
print("There are {} training images, {} testing images, and the validation split is {}% of training data, which is {:.0f} images.".format(len(train_images), len(test_images), valid_split*100, len(train_images)*valid_split))

#build model
cnn = Sequential()

#cnn.add(BatchNormalization(input_shape=train_images.shape[1:]))
...
cnn.add(Conv2D(filters=150, kernel_size=2, padding='same', activation='relu'))
cnn.add(MaxPooling2D(strides=2, padding='same'))
cnn.add(Dropout(0.4))
...
epochs = 20
batch_size = 64
learn_rate = 0.001 #if starting lr is 0.01, model doesnt train. Kinda interesting...
...
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.1, patience=1, verbose=1, min_lr=1*10**(-8))
#TODO add early stop callback if needed

#train model
time_to_train = time.time()
#history = cnn.fit_generator(augmented_data_gen.flow(train_images, train_labels, batch_size=batch_size, subset='training'),
#steps_per_epoch=train_images.shape[0] // batch_size, epochs=epochs, verbose=1, callbacks=[checkpointer, ten_board, reduce_lr],
#validation_data=augmented_data_gen.flow(train_images, train_labels, batch_size=batch_size, subset='validation'),
#validation_steps=(train_images.shape[0]*valid_split) // batch_size, use_multiprocessing=True)
...
history_save_path = "saved_histories/{}_cnn_history.pkl".format(start_time)
